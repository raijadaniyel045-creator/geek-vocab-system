  ...
  <div class="app-container">
    <header class="header">
      ...
      <div class="nav-segmented-control">
        <button :class="{ active: currentMode === 'stats' }" @click="switchTab('stats')">📊 数据看板</button>
        <button :class="{ active: currentMode === 'add' }" @click="switchTab('add')">✨ 生词录入</button>
        <button :class="{ active: currentMode.includes('review') }" @click="switchTab('review_setup')">🎯 专注复习
        </button>
        <button :class="{ active: currentMode === 'manage' }" @click="loadAllWords">📚 我的词库</button>
        <button :class="{ active: currentMode === 'ai_chat' }" class="ai-nav-btn" @click="switchTab('ai_chat')">🤖 AI
          洞察
        </button>
      </div>
    </header>
    ...
    <main v-if="currentMode === 'add'" class="card add-card fade-in">
      <div class="input-group">
        <input v-model="newWord" placeholder="输入单词，按回车搜索..." type="text" @keyup.enter="searchWord"/>
        <button :disabled="loading" class="primary-btn" @click="searchWord">
          {{ loading ? '🔍 搜索中...' : '🔍 搜索' }}
        </button>
      </div>
      ...
      <div v-if="previewWord" class="preview-box">
        <div class="preview-header">
          ...
          <button v-if="previewWord.audio_url" class="audio-btn" @click="playAudio(previewWord.audio_url)">🔊 播放原声
          </button>
        </div>
        <p class="phonetic">{{ previewWord.phonetic }}</p>
        <div class="editor-group">
          <label>💡 精简释义 (可直接修改并保存)</label>
          <input v-model="previewWord.chinese_meaning" class="modern-input" type="text"/>
        </div>
        <button class="confirm-btn" @click="confirmAddWord">✅ 确认录入词库</button>
      </div>
      <div v-if="message" :class="{'success': isSuccessMsg}" class="toast-message">{{ message }}</div>
    </main>
    ...
    <main v-if="currentMode === 'review_setup'" class="card setup-card fade-in">
      <div class="setup-form">
        <div class="form-section">
          ...
          <input v-model="reviewLimit" class="modern-range" max="50" min="5" step="5" type="range">
        </div>
        <div class="form-section">
          <label class="section-title">选择复习模式</label>
          <div class="radio-group">
            <label class="radio-label"><input v-model="reviewType" type="radio" value="flashcard"> 📇 经典翻卡</label>
            <label class="radio-label"><input v-model="reviewType" type="radio" value="spell_zh2en"> ✍️ 默写模式 (看中写英)</label>
            <label class="radio-label"><input v-model="reviewType" type="radio" value="spell_en2zh"> 🧠 释义挑战 (看英写中)</label>
          </div>
        </div>
        <button class="primary-btn start-btn" @click="startReview">🚀 开始复习</button>
      </div>
    </main>
    ...
    <main v-if="currentMode === 'manage'" class="card manage-card fade-in">
      <div class="manage-header">
        ...
        <button class="secondary-btn" @click="exportCSV">⬇️ 导出 CSV</button>
      </div>
      <div class="table-container">
        <table class="modern-table">
          ...
          <tr v-for="word in allWords" :key="word.id">
            ...
            <td><input v-model="word.chinese_meaning" class="table-input" title="修改后自动保存" type="text"
                       @blur="updateMeaning(word)"/></td>
            <td><span :title="'数值越高掌握越好，默认2.5'" class="ef-badge">{{
                word.easiness_factor ? word.easiness_factor.toFixed(1) : '2.5'
              }}</span></td>
            <td>
              <button class="text-danger-btn" @click="deleteWord(word.id)">删除</button>
            </td>
          </tr>
          </tbody>
        </table>
    ...
    <main v-if="currentMode === 'ai_chat'" class="card ai-chat-card fade-in">
      ...
      <div class="chat-input-wrapper">
        <textarea v-model="chatInput" placeholder="输入分析指令 (按 Ctrl+Enter 快捷发送)..."
                  rows="1" @keyup.ctrl.enter="sendChatMessage(chatInput)"></textarea>
        <button :disabled="globalAILoading" class="send-btn" @click="sendChatMessage(chatInput)">
...
const switchTab = (tab) => {
  currentMode.value = tab;
  previewWord.value = null;
  message.value = '';
  aiResponse.value = ''
...
const fetchStats = async () => {
  try {
    stats.value = (await axios.get(`${apiBase}/stats`)).data
  } catch (e) {
  }
}
...
const searchWord = async () => {
  ...
  previewWord.value = null
  try {
    previewWord.value = (await axios.get(`${apiBase}/search/${newWord.value}`)).data
  } catch (e) {
    message.value = "❌ 查询失败"
  } finally {
    loading.value = false
...
const confirmAddWord = async () => {
  try {
    await axios.post(`${apiBase}/words`, previewWord.value);
    message.value = "录入成功";
...
const startReview = async () => {
  currentMode.value = 'reviewing';
  showMeaning.value = false;
  userSpellInput.value = '';
  aiResponse.value = ''
  try {
    reviewList.value = (await axios.get(`${apiBase}/words/due?limit=${reviewLimit.value}`)).data
  } catch (e) {
  }
...
const submitScore = async (score) => {
  try {
    await axios.put(`${apiBase}/words/${currentReviewWord.value.id}/review`, {quality: score})
...
const askAI = async () => {
  ...
  aiResponse.value = "深度解析中..."
  try {
    aiResponse.value = (await axios.post(`${apiBase}/ai/tutor`, {
      word: currentReviewWord.value.word,
      question: "给个记忆口诀"
    })).data.answer
  } catch (e) {
    aiResponse.value = "解析失败"
  } finally {
...
const sendChatMessage = async (queryText) => {
  ...
  scrollToBottom()
  chatHistory.value.push({role: 'ai', content: '检索并推理中...'});
  scrollToBottom()
  try {
    ...
    chatHistory.value[chatHistory.value.length - 1].content = "请求失败"
  } finally {
...
const loadAllWords = async () => {
  switchTab('manage');
  try {
    allWords.value = (await axios.get(`${apiBase}/words/all`)).data
  } catch (e) {
  }
}
const updateMeaning = async (word) => {
  try {
    await axios.put(`${apiBase}/words/${word.id}`, {chinese_meaning: word.chinese_meaning})
  } catch (e) {
  }
}
const deleteWord = async (id) => {
  if (confirm("确定删除？")) {
    try {
      await axios.delete(`${apiBase}/words/${id}`);
...
.nav-segmented-control button:hover {
  color: #111827;
  background: rgba(255, 255, 255, 0.5);
}

.nav-segmented-control button.active {
  background: #ffffff;
  color: var(--primary);
...
.quick-prompts-clean button:hover {
  border-color: #8b5cf6;
  color: #8b5cf6;
  background: #f5f3ff;
}
